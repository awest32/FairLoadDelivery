    ...
    try
        t_upper = @elapsed begin
            ...
            elseif FAIR_FUNC == "gini"
                # Gini formal-CC MILP on fraction served (χ = pserved/pd). Reuses
                # the per-period permutation + indicator-constraint infrastructure
                # from Palma formal CC; differs only in the sort-position
                # coefficients (2i−n−1) and the n·Σχ_z=1 normalization.
