...
const SWEEP_STYLES = [
    # 2026-05-30 T=3 no-BD comparison: only the min-max trade-off was run, so
    # palma sweeps are commented out to avoid picking up stale T=24 results
    # from earlier dates. Re-enable when the palma trade-off is rerun at the
    # same T to overlay both curves.
