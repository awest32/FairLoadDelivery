...
function ac_feasibility_test(math::Dict{String, Any}, 
    ...
    for (set_id, bernoulli_set) in enumerate(bernoulli_selection_exp)
        for s in switch_ids
            ...
            math_round["switch"][string(s)]["status"] = round(Int, math_round["switch"][string(s)]["state"])
            math_round["switch"][string(s)]["dispatchable"] = 0
        end
        #pm_ivr_soln_round = solve_mc_pf(math_round, IVRUPowerModel, optimizer)
        pf_ivrup_aw = PowerModelsDistribution.solve_mc_opf(math_round, IVRUPowerModel, ipopt)#solve_mc_pf_aw(math_round, ipopt)
