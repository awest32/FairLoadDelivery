...
""
function solve_mc_mld_switch(data::Dict{String,<:Any}, solver; kwargs...)
    return _PMD.solve_mc_model(data, _PMD.LinDist3FlowPowerModel, solver, build_mc_mld_switchable; ref_extensions=[ref_add_load_blocks!], kwargs...)
end
...
function build_mc_mld_shedding_implicit_diff(pm::_PMD.AbstractUBFModels)
   ...
   # variable_mc_switch_indicator(pm; relax=true)
    ...
    #for i in _PMD.ids(pm, :load)
        constraint_load_shed_def(pm)
   # end
...
function build_mc_mld_switchable(pm::_PMD.AbstractUBFModels)
    ...
    for i in _PMD.ids(pm, :switch)
        constraint_mc_switch_state_on_off_top(pm, i)
        #_PMD.constraint_mc_switch_state_on_off(pm, i)
        # The switch thermal limit is not implemented in PowerModelsDistribution yet
        _PMD.constraint_mc_switch_thermal_limit(pm, i)
